...
const StoryCard = ({ item, index }) => {
  return (
    <div className="group relative rounded-2xl border bg-white dark:bg-gray-900 dark:border-gray-700 p-6 overflow-hidden">
      <div className="absolute -top-10 -right-10 w-28 h-28 rounded-full bg-rose-100 dark:bg-gray-800 group-hover:scale-125 transition-transform" />
      <div className="relative">
        ...
        <h3 className="mt-3 text-lg font-bold text-pink-950 dark:text-white">{item.title}</h3>
  ...
  return (
    <section className="px-2 md:px-0">
      <div className="flex items-center justify-between mb-4">
        <h2 className="text-2xl md:text-3xl font-extrabold text-pink-950 dark:text-white">Our Promise</h2>
        <a href="/user-details/?layout=3" className="text-rose-700 hover:underline dark:text-rose-300">Contact us</a>
      </div>
